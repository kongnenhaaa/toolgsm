...
public class SmsMultipartAssemblerTests
{
    ...
    [InlineData(67)]
    [InlineData(153)]
    public void QcmgrWithoutConcatMetadata_AlwaysRetriesInPduMode(int length)
    {
        string content = new('A', length);
        DecodedSmsBody decoded = SmsBodyDecoder.Decode(
            $"+QCMGR: \"REC UNREAD\",\"888\",,\"26/08/13,18:00:00+28\"\r\n{content}\r\nOK\r\n");

        Assert.Null(decoded.Concatenation);
        Assert.True(GsmModemService.ShouldRetryQcmgrInPduMode(decoded));
    }

    [Fact]
    public void QcmgrWithExactConcatMetadata_DoesNotNeedPduRetry()
    {
        DecodedSmsBody decoded = SmsBodyDecoder.Decode(
            "+QCMGR: \"REC UNREAD\",\"888\",,\"26/08/13,18:00:00+28\",4660,2,3\r\nphan thu hai\r\nOK\r\n");

        Assert.Equal(new SmsConcatInfo(4660, 3, 2), decoded.Concatenation);
        Assert.False(GsmModemService.ShouldRetryQcmgrInPduMode(decoded));
    }
